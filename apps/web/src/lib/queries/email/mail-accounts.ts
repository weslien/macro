...
import { getGraphqlSoupClient, graphqlCacheEnabled } from '@service-storage/graphql-soup';
...
export function useMailAccountsQuery() {
  ...
  const cached = createUrqlQuery(() => ({
    ...
    select: (data) => ({ links: data.user.emailLinks.map((link) => ({
      id: link.id, email_address: link.emailAddress, photo_url: link.photoUrl,
    })) }),
  }));
  return { get data() { return (!cached.isLoading ? cached.data : undefined) ?? (rest.isSuccess ? rest.data : undefined); } };
}
